...
class InstanceLabelResizeLayer(nn.Module):
    ...
    def forward(self, x,need_backprop):
        ...
        resized_lbs = np.zeros((feats.shape[0], ), dtype=np.float32)
        for i in range(lbs.shape[0]):
            resized_lbs[i*self.minibatch:(i+1)*self.minibatch] = lbs[i]

        y=torch.from_numpy(resized_lbs).float().cuda()

        return y

class ImageFBlabelResizer(nn.Module):
    def __init__(self):
        super(ImageFBlabelResizer, self).__init__()

    def forward(self, x, need_backprop, anchor_label):
        height = x.size(2)
        width = x.size(3)

        a_height = anchor_label.size(1)
        a_width = anchor_label.size(2)

        return anchor_label.long()
