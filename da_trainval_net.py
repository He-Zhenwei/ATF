...
if __name__ == '__main__':
  ...
  params = []
#  a = dict(fasterRCNN.named_parameters())
...
#        params += [{'params':[value], 'lr':lr* 10, 'weight_decay': cfg.TRAIN.WEIGHT_DECAY}]
  ...
  if args.resume:
    load_name = os.path.join(output_dir,
      'cityscape_{}_{}.pth'.format(args.checksession, args.checkepoch))
  ...
  for epoch in range(args.start_epoch, args.max_epochs + 1):
    ...
    for step in range(iters_per_epoch):
      ...
      rois_label,DA_img_loss_cls, tgt_DA_img_loss_cls, DA_ins_loss_cls,tgt_DA_ins_loss_cls =\
      ...
      loss = rpn_loss_cls.mean() + rpn_loss_box.mean() \
           + RCNN_loss_cls.mean() + RCNN_loss_bbox.mean() \
             +args.lamda*(7* DA_img_loss_cls.mean()+DA_ins_loss_cls.mean()\
             +7* tgt_DA_img_loss_cls.mean()+tgt_DA_ins_loss_cls.mean())
      loss_temp += loss.item()
    ...
    if epoch==args.max_epochs or (epoch % 1 == 0):
        save_name = os.path.join(output_dir, 'cityscape_%d_%d.pth' % (args.session, epoch))
        save_checkpoint({
    ...
    if epoch==args.max_epochs:
        save_name = os.path.join(output_dir, 'cityscape_consist_default.pth')
        save_checkpoint({
            'session': args.session,
            'epoch': epoch + 1,
            'model': fasterRCNN.module.state_dict() if args.mGPUs else fasterRCNN.state_dict(),
